...
``report`` (the free-text cardiologist report) is consumed downstream by the
teacher input pipeline (``src/teacher``), which cleans it before the teacher sees
it. ``heart_axis`` and ``infarction_stadium1/2`` are carried for the same
pipeline so nothing downstream re-parses the raw CSVs.
"""
        ...
        records.append(
            {
                ...
                "sex": int(row["sex"]),
                # Recording properties consumed by the teacher view (Phase 1.4).
                # Raw PTB-XL codes/labels; empty CSV cells -> None. The infarction
                # stadium is annotated in two independent slots.
                "heart_axis": _nan_to_none(row["heart_axis"]),
                "infarction_stadium1": _nan_to_none(row["infarction_stadium1"]),
                "infarction_stadium2": _nan_to_none(row["infarction_stadium2"]),
